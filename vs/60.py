...
class ColorAnalyzer:
    ...
    def analyze(self, image_path):
        # Improved skin detection and clustering
        ...
        dominant = colors[0]
        # Map to a season (simple logic for demo)
...
class FlawntApp:
    ...
    def create_home(self):
        ...
        for color in ['#7B3F00', '#A9746E', '#8B5C2A', '#A0522D', '#6B4226']:
         swatch = tk.Label(palette_frame, width=4, height=2, bg=color)
    ...
    def open_camera(self):
        ...
        self.img_preview = tk.Label(frame, width=220, height=260, bg='#e0e0e0')

        self.img_preview.pack(pady=10)
        btn_frame = tb.Frame(frame, bootstyle='light')
        btn_frame.pack(pady=10)
        tb.Button(btn_frame, text='Retake', bootstyle='secondary', width=12, command=self.open_file).pack(side=LEFT, padx=8)
    ...
    def open_file(self):
        ...
        if file_path:
            ...
            img = Image.open(file_path).resize((220, 260))
            img_tk = ImageTk.PhotoImage(img)
            self.img_preview.config(image=img_tk)
            self.img_preview.image = img_tk
            self.selected_img = file_path
    ...
    def confirm_image(self):
        ...
        if not self.analysis:
            messagebox.showerror('Error', 'Could not analyze the image. Try another photo.')
            return
